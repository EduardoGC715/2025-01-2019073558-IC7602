    ...
    try {
      ...
      if (success) {
        ...
        if (verificationResult.success) {
          ...
          Object.entries(verificationResult.results).forEach(
            ([domainName, result]) => {
  ...
  const handleVerifyDomain = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const { success: domainsSuccess, domains: newDomains } =
              ...
              <table className="min-w-full text-sm text-left">
                ...
                <tbody>
                  {Object.entries(domains).map(
                    ([domainName, domainData], index) => (
                      <tr
                        key={domainName}
                        className="border-b hover:bg-lightgrey2 transition-colors duration-200"
                      >
                        ...
                        <td className="px-4 py-2">
                          <button
                            onClick={() =>
                              navigate(`/domains/${domainName}/subdomains`)
                            }
                            className="text-accentBlue hover:text-secondary hover:cursor-pointer transition-colors duration-200"
                          >
                            Ver Subdominios
                          </button>
                        </td>
                        ...
                        <td className="px-4 py-2">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="p-1 hover:bg-lightgrey2 hover:cursor-pointer rounded-full transition-colors duration-200"
                              disabled={isLoading}
                            >
                              <RefreshCw
                                size={16}
                                ...
                                ${
                                  domainData.validated
                                    ? "text-lightgrey hover:text-lightgrey1"
