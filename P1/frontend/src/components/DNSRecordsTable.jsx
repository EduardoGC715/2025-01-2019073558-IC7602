...
const DNSRecordsTable = ({ dnsRecords, loading, onEditRecord, onDeleteRecord }) => {
  // ── real-time Firebase snapshot of all domains ─────────
  ...
  const getExtraCol = policy => {
    if (policy === "weight") return { label: "Peso", key: "weight", badgeBg: "info" };
    if (policy === "geo")    return { label: "Región", key: "country", badgeBg: "secondary" };
    return null;
  };
  ...
  const renderDirections = (record) => {
    ...
    if (record.type === "multi" || record.type === "round-trip") {
      ...
      return record.direction.split(',').map(s => s.trim()).filter(Boolean).map(pair => {
          const [ip, weight] = pair.split(':');
          return (
            <div key={ip} className="d-flex align-items-center" style={{ gap: '10px' }}>
              {ip}
              <span>-</span>
              Peso: {weight}
            </div>
      ...
      <Col>
        <Card>
          ...
          <Card.Body>
            ...
            {showModal && (
            <Modal show={true} onHide={() => setShowModal(false)} size="lg" centered>
                  ...
                  const entries = raw.ip
                    ...
                    : raw.ips
                      ? Array.isArray(raw.ips)
                        ? raw.ips.map(ip => ({ ...ip }))                   // weight entries already have .weight
                        : Object.entries(raw.ips).map(([country, ip]) => ({ ...ip, country }))
                      : [];
                    return (
                        ...
                        <tbody>
                          {entries.map(ipEntry => (
                            <tr key={ipEntry.address}>
                              ...
                              <td>
                                <Badge bg={ipEntry.health ? 'success' : 'danger'}>
                              ...
                              <td>
                                {ipEntry.healthcheck_results && Object.keys(ipEntry.healthcheck_results).length > 0 ? (
                                  <Table size="sm" borderless>
                                    ...
                                    <tbody>
                                      {Object.entries(ipEntry.healthcheck_results).map(
                                        ([checker, res]) => (
                                          <tr key={checker}>
                                            <td>{checker}</td>
                                            <td>{res.duration_ms.toFixed(2)} ms</td>
                                            <td>
                                              <Badge bg={res.success ? "success" : "danger"}>
