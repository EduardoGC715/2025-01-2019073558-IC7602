...
class AudioPlayer:
    def __init__(self, atm_file):
        ...
        self.audio_array = np.concatenate(self.frames) if self.frames else np.array([], dtype=np.int16)

        # FFT data guardada (opcional, si deseas usarla)
    ...
    def update_plot(self):
        ...
        if toolbar is not None and toolbar.mode != "":
            ...
            self.ax_time.set_ylim(
                np.min(audio_segment) - 500, 
                np.max(audio_segment) + 500
            )
